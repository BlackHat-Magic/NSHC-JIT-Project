...
fn main() {
    ...
    let max_memory: usize = max_memory_words.try_into().expect("Maximum memory too large.")

    let min_memory_kib: u64 = *matches.get_one::<u64>("min_memory").unwrap();
    let min_memory_words: u64 = min_memory_kib * 256 & 0xFFFFFFFF;
    let min_memory: usize = min_memory_words.try_into().expect("Minimum memory too large.")
